...
**/
	...
	else return FALSE; /* fix warning about end of non-void function without return */
} 
...
**/
	...
	if ((*RootPtr) == NULL) {
		tBSTNodePtr new_item;
		new_item = (struct tBSTNode*)malloc(sizeof(struct tBSTNode));
		new_item->Key = K;
		new_item->BSTNodeCont = Content;
		new_item->LPtr = NULL;
		new_item->RPtr = NULL;
		(*RootPtr) = new_item;
	}
...
**/
	...
	if ((*RootPtr)->RPtr == NULL) { /* Most rigt found */
		PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
		PtrReplaced->Key = (*RootPtr)->Key;
		tBSTNodePtr temp = (*RootPtr);
		(*RootPtr) = (*RootPtr)->LPtr; /* If have one child, parent will inherit it */
		free(temp);
	}
	else {
...
**/
	...
	if ((*RootPtr) != NULL) {
		if ((*RootPtr)->Key > K) {
			...
			tBSTNodePtr temp = (*RootPtr);
			if (temp->RPtr == NULL) { /* If have one child, parent will inherit it */
				(*RootPtr) = temp->LPtr;
				free(temp);
			}
			else { 
				if (temp->LPtr == NULL) { /* If have one child, parent will inherit it */
					(*RootPtr) = temp->RPtr;
					free(temp);
				}
				else { /* Have two children */
					ReplaceByRightmost((*RootPtr), &temp->LPtr);
				}
			}
		}
	}
...
**/
	...
	if(NodePtr == NULL && IsRoot) { /* Empty tree */
		return -1;
	}
	else if (NodePtr->LPtr == NULL && NodePtr->RPtr == NULL) { /* NodePtr is leaf */
		return 0;
	}
	if (NodePtr->LPtr != NULL) {
		lh = BSTHeight(NodePtr->LPtr, FALSE);
	}
	if (NodePtr->RPtr != NULL) {
		rh = BSTHeight(NodePtr->RPtr, FALSE);
	}
	if (NodePtr->RPtr == NULL && NodePtr->LPtr != NULL) {
		return lh +1;
	}
	else if (NodePtr->RPtr != NULL && NodePtr->LPtr == NULL) {
		return rh +1;
	}
	else if(lh > rh)
		return lh + 1;
