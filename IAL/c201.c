...
***/
    ...
    tElemPtr temp = L->First;   /* Prave uvolnovany prvok */
    tElemPtr ntemp = temp;      /* Prvok, kt. nasleduje za uvolnovanym prvkom */
...
**/
    ...
    if ((temp = malloc(sizeof(struct tElem))) == NULL)  Error();
    else {
        temp->data = val;
        if (L->First == NULL) {
            temp->ptr = NULL;   
            /* ^^^
             * Ak je zoznam na zaciatku rpazdny je potrebne nastavit 
             * nasledujuci prvok na NULL, pretoze pri inicializacii 
             * to nie je mozne 
             */
        }
        else temp->ptr = L->First;
        L->First = temp;
    }
}
...
**/
    ...
    if (L->First != NULL) {
        ...
        temp = L->First->ptr;   /* temp ukazuje na prvok za prvym prvkom */
        free(L->First);
        L->First = temp;
    }
}	
...
**/
    ...
    if (L->Act != NULL)
    if (L->Act->ptr != NULL) {
        temp = L->Act->ptr->ptr;    /* Ukazuje na druhy prvok za aktivnym */
        free(L->Act->ptr);          /* Uvolni prvok za aktivnym */
        L->Act->ptr = temp;
    }
}
...
**/
    ...
    if (L->Act != NULL) {
        ...
        temp->ptr = L->Act->ptr;    
        /* ^^^
         * Prvok za pridanym je prvok, kt bol za aktivnym 
         */
        L->Act->ptr = temp;
    }
}
